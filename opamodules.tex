...
\desc{Linear beam optics includes closed orbit finder, periodic solution for coupled and uncoupled lattices, and full linear optics including integrals with several options for periodic, symmetric, single pass forward, backward, from marker calculations. 
...
Plot routines show beta functions and dispersions, or orbit, envelopes and apertures, or magnetic fields. Lattice and local beam parameters are filled to the tables, and several output files can be printed. 
}
...
\pvar{
Several variables to be used by \ogui{opticview}, \ogui{Opticstart} etc. were defined in the interface section, i.e. as public, for convenience (i.e. lazyness). Most of them are flags to control the flow. Few of them may require an explanation:
...
\code{>PlotOrbit, PlotEnv, PlotApertures},\code{>PlotMag}.
\code{>Print...}\\
