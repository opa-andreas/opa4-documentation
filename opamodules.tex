...
\tabouni{OPAglobal} & & global variables and procedures & 2986 \\
...
\hline
\tabouni{opalatticefiles} & & file reading and writing & 1839 \\
\tabogui{texteditor} & \guico{TFormTxtEdt} & text editor for lattice file & 190 \\
\tabogui{OPAEditor} & \guico{TFormEdit} & interactive lattice editor  & 346 \\
\tabogui{EdElCreate} & \guico{TEditElemCreate} & creation of an element & 144 \\
\tabogui{EdElSet} & \guico{TEditElemSet} & edit element parameters & 861 \\
\tabogui{EdSgSet} & \guico{TEditSegSet} & edit segment parameters & 444 \\
\hline
\tabogui{opticview} & \guico{Toptic} & linear optics design & 1143 \\
\taboguif{knobframe} & \guifco{TKnob} & slider to set element property & 421 \\
\tabogui{Opticstart} & \guico{Tstartsel} & setting for start parameters & 615 \\
\tabogui{OpticEnvel} & \guico{TsetEnvel} & setting for optical functions plot & 372 \\
\tabogui{OpticTune} & \guico{TtuneMatrix} & adjustment of lattice tune & 251 \\
\tabogui{OpticWOMK} & \guico{TWOMK} & write optics markers & 125 \\
\tabouni{opticplot} & & linear beam dynamics calculations & 2840 \\
\tabouni{OPAElements} & & element propagation calculations & 2387 \\
\tabogui{OpticMatch} & \guico{TMatch} & linear optics matching & 1457 \\
\tabogui{OpticMatchScan} & \guico{TsetMatchScan}& parameter scan & 119 \\
\tabogui{OPAtune} & \guico{TTunePlot} & show the tune diagram & 634 \\
\hline
...
\tabogui{Bucket} & \guico{TBucketView} & plot RF bucket & 640 \\
\hline
\tabogui{OPAChroma} & \guico{TChroma} & nonlinear optimization & 1413 \\
\taboguif{CSexLine} & \guico{TCSex} & controller for nonlinear element & 454 \\
\taboguif{CHamLine} & \guico{TCHam} & bar indicator for Hamiltonian mode & 416 \\
\tabouni{ChromGUILib1} & & little helper to pass results handles & 79 \\
\tabouni{ChromGUILib2} & & little helper to pass element handles & 93 \\
\tabogui{OPAChromaSVector} & \guico{TSVectorPlot} & show Hamiltonian as complex vector & 145 \\
\tabouni{chromlib} & & nonlinear dynamics calculations & 1813 \\
\hline
\tabogui{OPAorbit} & \guico{TOrbit} & orbit correction and injection & 2353 \\
\hline
\tabogui{OPAtrackP} & \guico{} & phase space tracking & 1422 \\
\tabogui{OPAtrackDA} & \guico{} & dynamic aperture tracking & 1666 \\
\tabogui{OPAtrackT} & \guico{} & Touschek tracking & 1726 \\
\tabouni{tracklib} & & particle tracking calculations & 1502 \\
\hline
\tabogui{LGBeditor} & \guico{} & longitudinal gradient bend editor & 702 \\
\tabouni{LGBeditorLib} & & longitudinal gradient bend calculations & 471 \\
\hline
...
\tabouni{opatest} & & test of new features & 472 
\end{tabular}
}
\end{table}
...
\unico{setOrbitHandle} receives a handle to the \ogui{OPAorbit} GUI. Actually it is only needed to send it a \code{repaint} event, it will then get and plot the data from \ouni{opticplot}.\\
...
\unico{OpticCalc} performs the full linear optics calculation by calling \unico{ClosedOrbit}, \unico{Periodic} and \unico{LinOp}. Afterwards, the working point is plotted to the tune diagram (in case of periodic solution only) and the table in \ogui{opticview} is filled calling procedures \unico{FillBeamTab} and \unico{FillBetaTab}. \\
...
\guifco{UpdateWeight} accepts a new weight factor and adjusts the scale of {\em all} \oguif{CHamLine} instances to maintain comparability of the bars. 
...
\ogui{Opticstart} is called in both modes to set the initial conditions, where the periodic option is disabled in injection mode, because it makes no sense.

\todo{\ogui{OPAorbit} is similar to \ogui{opticview} for linear optics design, because both show the orbit and provide clickable elements to be dragged to knobs. Furthermore, data are transferred from one to the other to calculate the optics on a corrected orbit. Thus it would make sense to combine both GUIs into one, perhaps using tabs for the different modes. However, \ogui{OPAorbit} uses two \oguif{../com/asfigure} instances for plotting and the clickable elements are realized as \code{TShape} objects on the form, underneath the plot, whereas \ogui{opticview} has its own, much more complex plot window with bars inside the plot for the elements.}
}

\uses{
\oguif{knobframe},  \ouni{opticplot}, \ouni{Opticstart}, \ouni{OPAglobal}, \ouni{mathlib}, \oguif{../com/asfigure}, \ouni{../com/asaux}
}

\ppro{
\guico{Start} receives a flag to set orbit or injection mode. Then the form passes a handle to itself to \ogui{opticplot} to enable a repaint. The girders are set up calling \prcod{OPAglobal}{GirderSetup}. Then the lattice is searched for correctors, resp. kickers and monitors, arrays are declared to manage them, and symbols are created on the form. After restoring user settings and filling edit fields, \ogui{Opticstart} is launched to get started.
}
...
\evcodx{repaint}{FormPaint} plots the orbit. The procedure is called by \prcod{opticplot}{PlotOrbit} via the handle to the form passed at initialization. Orbit calculation and plotting is triggered by  \ogui{Opticstart} and also by some components in the GUI. Plotting is done by private procedure \guico{Makeplot} which calls several other private procedures: \guico{Get (Orbit, Cor, Mis) Max} get the maxima of data for plot scaling, \guico{plot(X,Y)} plot the orbit, \guico{WeightPlot} shows a histogram of orbit correction SVD weighting factors, \guico{BPMshow} fills the BPM readout panel,  \guico{LabelShow} updates the correction status and \guico{CodStat} fills a panel with orbit statistics data. In injection mode some of these procedures do nothing.
...
\evcod{$<\!\!-\!\!>$ , $>\!\!-\!\!<$ , $--\!\!>$ , $<\!\!--$}{butzooClick} are zoom functions, similar but simpler than in \ogui{opticview}.
...
\evcod{Start}{butStaClick} launches the \ogui{Opticstart} start panel again to change initial conditions etc. \todo{BUG: plot error when starting from an optics marker.}
...
\evcod{Sync}{butSyncClick} synchronizes the kicker by calculating the time of flight from the start of the lattice to the entry to the kicker and sets its field \unico{delay} to the same value. If the kicker is of finite length, \prcod{OPAElements}{Kicker} takes care of internal time of flight.

\evcod{$-$,$+$}{butKickT(m,p)Click} increase the number of turns (max 3) to be calculated and plotted in a circular lattice. \prcod{opticplot}{OrbitCalc} is called with a negative number used as a switch to count turns and start at begin of lattice in successive turns.
...
\uses{\ouni{tracklib}, \ouni{OPAglobal}, \ogui{OPAtune}, \ouni{mathlib}, \oguif{../com/asfigure}, \oguif{../com/Vgraph}, \ouni{../com/asaux}}
...
\evcod{Beam}{butBeamClick} sets panel \guico{PanBeam} visible (and hides \guico{PanTush}, or vice versa) to enter beam parameters for a particle ensemble centered at the initial coordinates.
...
\uses{\ouni{tracklib}, \ouni{OPAglobal} \ouni{mathlib}, \oguif{../com/asfigure}, \oguif{../com/Vgraph}, \ouni{../com/asaux}}
...
\uses{\ouni{tracklib}, \ouni{opticplot}, \ouni{OPAglobal} \ouni{mathlib}, \oguif{../com/asfigure}, \oguif{../com/Vgraph}, \ouni{../com/asaux}}
...
\uses{\ouni{opticplot}, \ouni{OPAElements}, \ouni{OPAglobal} \ouni{mathlib}}
